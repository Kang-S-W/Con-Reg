...
<style>
    ...
    div[data-testid="stExpander"] details > div { background-color: #ffffff !important; color: #334155 !important; }
    ...
    with history_container:
        if st.session_state.chat_history:
            for i, chat in enumerate(reversed(st.session_state.chat_history)):
                ...
                col_btn, col_del = st.columns([78, 22])
                with col_btn:
        ...
        with col_state:
            ...
            with st.container(border=True):
                ...
                if st.button("🔗 토지대장 정보 연동", use_container_width=True):
                    if current_chat is not None:
                        current_chat["state"].update({"검토지구": "일반주거지역", "제한구역": "비행안전구역", "산정대지면적": "1,250㎡"})
                        save_history(st.session_state.chat_history, st.session_state.user_id)
                        st.rerun()
            ...
            if current_chat is not None:
                ...
                st.caption("ℹ️ 현재 활성화된 세션 대화방이 없습니다. 좌측에서 대화를 시작하거나 아카이브를 선택하십시오.")
